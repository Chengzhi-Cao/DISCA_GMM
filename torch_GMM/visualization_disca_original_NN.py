...
# from torch_disca_original_tfGMMUv2 import YOPO_Final_Model, YOPOClassification,YOPOFeatureModel
from torch_disca_original_tfGMMUv7 import YOPOFeatureModel,YOPOClassification
...
# #stored path
# parser.add_argument("--saving_path", type=str, default='/data/zfr888/EMD_4603/Results8/')
# parser.add_argument("--algorithm_name", type=str, default='gmmu_cavi_llh_hist')
# parser.add_argument("--filtered_particle_saving_path", type=str, default='/data/zfr888/EMD_4603/Results8/filtered_particle')
...
# parser.add_argument("--subtomo_num",type=int, default=16260)
# parser.add_argument("--subtomo_num_test",type=int, default=16260)
...
parser.add_argument("--data_path", type=str, default='/data/zfr888/EMD_4603/data_emd4603/original')
#stored path
parser.add_argument("--saving_path", type=str, default='/data/zfr888/EMD_4603/Results_torch_original0912v7')
...
for model_name in model_names:
    ...
    # combined_model = YOPO_Final_Model(model_feature, model_classification).to(Config.device)


    model_feature.load_state_dict(torch.load(combined_path))
    model_classification.load_state_dict(torch.load(classmodel_path))
    ...
    with torch.no_grad():
        for a in range(total_num):
            ...
            labels_combined = model_classification(features)

            # labels_combined = combined_model(x_train)   # x_train=[100,1,24,24,24]
            
            labels = np.array([np.argmax(labels_combined[q, :].cpu().detach().numpy()) for q in range(len(labels_combined))])
            # print('labels=',labels)
