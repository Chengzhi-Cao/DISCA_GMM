...
#stored path
parser.add_argument("--saving_path", type=str, default='/data/zfr888/EMD_4603/Results_tf0928')
parser.add_argument("--algorithm_name", type=str, default='gmmu_cavi_llh_hist')
parser.add_argument("--filtered_particle_saving_path", type=str, default='/data/zfr888/EMD_4603/Results_tf0928/filtered_particle')
...
for scanning_it in range(scanning_num):
    ...
    for f in sorted(os.listdir(data_path)):   
        ...
        for i in range(factor):         # factor=40 
            for j in range(factor):           
                for k in range(factor):       
                    ...
                    subvolumes.append(np.expand_dims(np.array(subvolume), [0,-1]))  # subvolumes=64000*[1,46,46,34,1], [1,115,115,69,1]
        ...
        for subv_i in tqdm(range(len(subvolumes))):
            ...
            subvolumes_label.append(subt_label) # subt_label=[1,92,92,46,9]
