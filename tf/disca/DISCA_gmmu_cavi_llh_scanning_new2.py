...
#    features_pca = pca.fit_transform(features) 
    ...
    if K in candidateKs:
        ...
        gmm = GMM(n_cluster = K_temp, a_o = None, b_o = None, u_filter = u_filter, u_filter_rate = u_filter_rate,reg_covar=reg_covar,init_param="kmeans") 
        
        gmm.fit(features_pca,max_iter=10) 
        labels_k = gmm.hard_assignment()

        # print('set(labels_k)=',set(labels_k))# set() 将集合中重复的元素去除
        # print('## Cluster sizes:', [np.sum(labels_k == k) for  k in set(labels_k)])  # 返回每个标签的数量
...
def YOPO_classification(num_labels, vector_size=1024):
    ...
    mod = tf.keras.Model(inputs=[main_input, positive_input, negative_input], outputs=[m1, m2, m3])
