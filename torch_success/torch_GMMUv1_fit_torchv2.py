...
if __name__ == '__main__':  
    ...
    for scanning_it in range(scanning_num):
        ...
        for f in sorted(os.listdir(data_path)):   
            ...
            # x_interval_start = np.linspace(0, 777, num=factor).astype(int)
            # x_interval_end = np.linspace(111, 888, num=factor).astype(int)
            # y_interval_start = np.linspace(0, 777, num=factor).astype(int)
            # y_interval_end = np.linspace(111, 888, num=factor).astype(int)
            # z_interval_start = np.linspace(0, 777, num=factor).astype(int)
            # z_interval_end = np.linspace(111, 888, num=factor).astype(int)
            ...
            # v = (v - np.mean(v))/np.std(v)
            # v_i = np.zeros_like(v)      # v_i=[928,928,464]
            # pp_index = pp_index.astype(int)
            # for j in range(len(pp_index)):              # j包含一个三维坐标, s=16, 
            #     v_i[pp_index[j][0],pp_index[j][1],pp_index[j][2]] = v[pp_index[j][0],pp_index[j][1],pp_index[j][2]]
            #     # 相当于对一个32x32x32的体素进行赋值
            # save_png(cub_img(v_i[:,:,::15])['im'], os.path.join(figure_path, 'scan_label_0' + '.png'))
