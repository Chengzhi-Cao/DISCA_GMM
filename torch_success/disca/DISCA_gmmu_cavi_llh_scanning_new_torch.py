...
def statistical_fitting_torch_split_merge(features, labels, candidateKs, K, reg_covar, it, u_filter = True,\
                                        u_filter_rate=0.0025, alpha = 1.0):
    '''
    introduce the torch version GMM
    '''
#    pca = PCA(n_components=32)  
#    features_pca = pca.fit_transform(features) 
    ...
    if K in candidateKs:
        ...
        for i in range(1, K+1): #ignore uniform
            ...
            gmm_i = GaussianMixture(n_components=2, covariance_type='full', \
                                    tol=0.001, random_state=i, init_params = 'random', reg_covar=reg_covar)
            gmm_i.fit(x_i) 
            ...
            # r_sub = np.expand_dims(r_all.cpu().detach().numpy(), axis=-1) * gmm_i.predict_proba(features_pca)  # (n, 2)
            r_c1 = r_sub[:, 0]  # (n, )
            r_c2 = r_sub[:, 1]  # (n, )
        ...
        gmm = GMM(n_cluster = K_temp, a_o = None, b_o = None, u_filter = u_filter, 
                  u_filter_rate = u_filter_rate,reg_covar=reg_covar,init_param="kmeans") 
        
        gmm.fit(features_pca,max_iter=10) 
        labels_k = gmm.hard_assignment()
...
def YOPO_classification(num_labels, vector_size=1024):
    ...
    m1 = dense(main_input)      # m1=[None,None,None,None,32]
    print('m1=',m1)
    print('m1.shape=',m1.shape)
    m2 = dense(positive_input)  # m2=[None,None,None,None,32]

    m3 = dense(negative_input)  # m3=[None,None,None,None,32]
