...
class TORCH_CAVI_GMMU(nn.Module):
    ...
    def _init_param(self, x):
        ...
        self.u_chi2_quantile = torch.tensor(1-self.u_filter_rate)
        #Priors for Uniform
        ...
        if self.init_param in [ "gmm", "self_setting"]:
            mat = []
            for i in range(self.k):
                ...
                _num = _val1 @ _val2 / _val3
                det = np.linalg.det(_num)
                if det == 0:
                    print("It is singular!")
                else:
                    print("It is nonsingular!")

                mat.append(
                    np.linalg.inv(_val1 @ _val2 / _val3) / lambda_nu_var[i])
        ...
        self.w_o = torch.tensor(w_o, dtype=self.DATA_TYPE)
    ...
    def update_lambda_w(self, lambda_w, Nk, Sk, xbar):
        ...
        Sk_g = Sk[1:]
        K = self.k 
        inv_w_o = torch.linalg.inv(self.w_o).to(device)
        ...
        for k in range(K):
            ...
            lambda_w[k, :, :] = torch.linalg.inv(_val.cpu()).to(device)
...
# /home/lab2/zyc/A_orange/DISCA_GMM
    def elbo(self, xn):
        ...
        logdet = torch.tensor([torch.logdet(self.lambda_w[i, :, :].cpu()) for i in range(self.k)], dtype=self.DATA_TYPE).to(device)
        ...
        product = torch.squeeze(torch.unsqueeze((torch.unsqueeze(torch.tensor(xn), axis=1).to(device) - self.lambda_m), axis=-2) @ self.lambda_w @ \
        torch.unsqueeze((torch.unsqueeze(torch.tensor(xn), axis=1).to(device) - self.lambda_m), axis=-1))
        ...
        '''
        # print("8")
        traces = torch.tensor([torch.trace(torch.matmul(torch.linalg.inv(self.w_o+ torch.eye(self.reg_covar *self.w_o.shape[1], dtype=self.DATA_TYPE).to(device)),self.lambda_w[K,:,:].to(device))) for K in range(self.k)]).to(device)
        # print("7")
    ...
    def init_KMeans_lambda_phi(self, x, n_centers, init_times=100, min_delta=1e-3, init_center = None):
        ...
        if init_center is None:
            for i in range(init_times):
                # x = x.cpu().detach().numpy()
                tmp_center = x[random_state.choice(torch.arange(x.shape[0]), size=n_centers, replace=False), ...]
                # tmp_center = x[random_state.choice(np.arange(x.shape[0]), size=n_centers, replace=False), ...]
                tmp_center = torch.tensor(tmp_center)
                l2_dis = torch.norm(torch.tile(torch.unsqueeze(x, 1), (1, n_centers, 1)) - tmp_center, p=2, dim=2)
                l2_cls = torch.argmin(l2_dis, axis=1)

                cost = 0
                for c in range(n_centers):
                    cost += torch.mean(torch.norm(x[l2_cls == c] - tmp_center[c], p=2, dim=1))
        ...
        while delta > min_delta:
            l2_dis = torch.norm(torch.tile(torch.unsqueeze(x, 1), (1, n_centers, 1)) - center, p=2, dim=2)
            l2_cls = torch.argmin(l2_dis, axis=1)
            center_old = torch.tensor(center, dtype=torch.double)

            for c in range(n_centers):
                if c == 0:
                    l2_cls = l2_cls.cpu().detach().numpy()
                    # x = x.cpu().detach().numpy()
                center[c] = torch.mean( torch.tensor(x[l2_cls == c]), axis=0)

            delta = torch.max(torch.sum(torch.square(center_old - center), axis=1))

        l2_dis = torch.norm(torch.tile(torch.unsqueeze(x, 1), (1, n_centers, 1)) - center, p=2, dim=2)
        l2_cls = torch.argmin(l2_dis, axis=1)
        return torch.tensor(l2_cls, dtype=self.DATA_TYPE), torch.tensor(center, dtype=self.DATA_TYPE)
    ...
    def fit(self, x, max_iter=10):  # x=[4,32]
        ...
        if self.init_param == "random":
            ...
            _min = torch.min(x_tf[:, 0])
            _max = torch.max(x_tf[:, 0])
            ...
            x = torch.tensor(x, dtype=self.DATA_TYPE).to(device)
            lambda_phi_var, c = self.init_KMeans_lambda_phi(x, self.k)
            _max = torch.max(x, dim=0)
            _val = _max.values
            lambda_m_var = c * (torch.max(x, dim=0).values - torch.min(x, dim=0).values) + torch.min(x, dim=0).values
            # add 1 for uniform
            self.lambda_phi =torch.tensor(0.01 / self.k * torch.ones((self.n, self.k + 1), dtype=self.DATA_TYPE),dtype=self.DATA_TYPE).to(device)
            for i, label in enumerate(lambda_phi_var):
                self.lambda_phi[i, int(label.cpu().detach().numpy()+1)] = (0.99)
